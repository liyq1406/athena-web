...
<t:page i18n="i18n.xqjs.diaobl.verifyzz">
	<t:panel id="panelA">
		...
		<t:grid id="grid_diaobsqmx" caption="${i18n.diaobsqmx}"
			...
			src="/diaobl/searchDiaobSqMx.ajax" afterParse="firstLoad"  
		...
		<t:form id="form_dbd" submit="NOT" reset="NOT">
			<t:grid idKeys="lingjbh,lux,usercenter,cangkbh" id="grid_diaobdmx"
				...
				usePager="true" load="false" src="/diaobl/searchDiaobdMx.ajax"
				...
				<t:gridCol property="cangkbh" caption="${i18n.cangkbh}"
					editorOptions="{closeEvent:func_set_cangkbh}" editor="fieldText"></t:gridCol>
				...
				<t:gridCol property="zhuangt" caption="${i18n.zhuangt}" convert="zhuangt_mx"></t:gridCol>
					
				<t:gridCol property="aaa" caption=""></t:gridCol>
			</t:grid>
		</t:form> 
				允许多次交付:<input type="radio" value="1" name="jf" checked="checked">Y 
				<input type="radio" name="jf" value="0">N
...
<script type="text/javascript"> 
	...
	//20170217-xss-v4_018
	$.sdcui.serverConfig.convertArray['zhuangt_mx'] = {
		'00' : '${i18n.notVerify}',
		'20' : '${i18n.yVerify}'
	};
	...
	function tongy() {
		...
		*/
		...
		//20170217-xss-v4_018
		var param = $('#grid_diaobdmx').grid('getSelectedRecords') == null ? [] : $(
				'#grid_diaobdmx').grid('getSelectedRecords'); 				
		...
		for ( var i = 0; i < param.length; i++) {
			...
			if (typeof cangkbh == 'undefined') {
				alert("${i18n.hangCangkNull}");
				return false;
			} else if (typeof shipsl != 'undefined'
					&& typeof shengxsj == 'undefined') {
				alert("${i18n.hangSxsjNull}");
				return false;
			} else if (typeof shipsl != 'undefined'
					&& typeof shifdbz == 'undefined') {
				alert("${i18n.hangBzNull}");
				return false;
			} else if (typeof shipsl != 'undefined') {
				var kcsl = typeof kucsl == 'undefined' ? "0" : parseInt(kucsl);
				//v4_018 还要判断小数点
				if(lux =='97X' || lux == '97D'){
					if (parseFloat(kcsl) < parseFloat(shipsl)) {
						alert("${i18n.spslNotdykcsl}");
						return false;
					}
				} 
		...
		$.sdcui.ajaxUtil.ajax({
			url : "${ctx}/diaobl/diaoblQr.ajax",//请求url
			data : params.join('&'),
			success : function(result) {
				alert(result.result);
	...
	function juj() {
		...
		$.sdcui.ajaxUtil.ajax({
			url : "${ctx}/diaobl/diaoblJj.ajax",//请求url
			data : data,
			success : function(result) {
				alert(result.result);
				$('#grid_diaobsqmx').grid('reload');
	...
	function shipsl_onload() {
		...
		//20170218-xss-v4_18
		$.each(dbmxRow, function(i, n) {
			var  zhuangt = $(n).find('td:eq(10)').text(); 
			if (zhuangt == '已审核' ) { 
				sl = $(n).find('td:eq(7)').text();
				sl = sl === "" ? 0 : sl;
				shipsl += parseInt(sl);
