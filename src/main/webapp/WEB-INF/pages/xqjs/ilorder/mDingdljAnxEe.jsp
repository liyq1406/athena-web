...
<t:page>
	...
	<t:form id="ilquery" reset="NOT" submit="NOT">
		<t:grid id="ilorderEe" idKeys="id,dingdh,gongysdm,usercenter,edit_time,editor,p0fyzqxh" caption="" editAlias="DingdUpdate" src="/ilorder/queryDdLj.ajax" usePager="true" edit="NOT"
			...
			<t:gridCol property="bzlx" type="text" caption="包装类型"></t:gridCol>
			<t:gridCol property="bzrl" type="text" caption="包装容量"></t:gridCol>
	...
	<t:form id="ilAddDdlj" dialog="true"  caption="新增" submit="NOT">
		<t:fieldLayout prefix="add">
			...
			<t:fieldText property="gongysdm" caption="订货/线边仓库" width="150" readonly="true"></t:fieldText>
			<t:fieldText property="cangkdm" caption="仓库" width="150" maxLength="3"></t:fieldText>
			<t:fieldText property="lingjbh" caption="零件号" width="150" notNull="true" maxLength="10"></t:fieldText>
			<t:fieldText property="bzlx" caption="包装类型" width="150" notNull="true"      readonly="true"></t:fieldText>
			<t:fieldText property="bzrl" caption="包装容量"   width="150" notNull="true"      readonly="true"></t:fieldText>
...
<script type="text/javascript">
  ...
  $(function() {
		$('#add_lingjbh').bind('focusout', function() {
			...
			if(chullx=='M1' || chullx=='MJ' || chullx=='MV'){
			   if($('#add_cangkdm').fieldValue()==""){
			      alert("仓库编号不能为空");
			  	  return false;
			   }else{
				   gongysdm=$('#add_cangkdm').fieldValue();
			   }
			}
			if(lingjbh==''){
			   alert("零件编号不能为空！");
			...
			$.sdcui.ajaxUtil.ajax({
					...
					success : function(result) {
						if (result.total == 0) {
						    ...
						    if(chullx=='M1' || chullx=='MJ' || chullx=='MV'){
								$("#add_bzlx").fieldValue(result.result[0].usbzlx);
								$("#add_bzrl").fieldValue(result.result[0].usbzrl);
						    }else{
						        $("#add_bzlx").fieldValue(result.result[0].uclx);
								$("#add_bzrl").fieldValue(result.result[0].ucrl);
						    }
						}
					}
				});
   ...
   */
   ...
   function editJiaoy(value){
		...
		var bzrlIndex = $('td.bzrl:first').prevAll().length;
   ...
   function link_p0sl(event){
	...
	function  func_button_subAdd(){
      ...
      if(p0sl%bzrl>0||p1sl%bzrl>0||p2sl%bzrl>0||p3sl%bzrl>0){
         alert("请按包装取整");
         return ;
      }
      ...
      var id = $("#ilorderEe").find('div.grid-scroll tr[id]').attr("id");
	  var p0fyzqxh = id.substring(id.lastIndexOf(",")+1);
      params.push('dd[0].p0fyzqxh='+p0fyzqxh);
      //params.push('dd[0].dingdwhr='+dingdwhr);
      if($('#il_chullx').fieldValue()=='M1'||$('#il_chullx').fieldValue()=='MJ'||$('#il_chullx').fieldValue()=='MV'){
         params.push('dd[0].usbaozrl='+bzrl);
         params.push('dd[0].usbaozlx='+bzlx);
      }else{
          params.push('dd[0].uabzuclx='+bzlx);
          params.push('dd[0].uabzucrl='+bzrl);
      }
