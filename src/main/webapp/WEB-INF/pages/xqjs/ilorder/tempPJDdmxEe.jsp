...
<t:page>
	...
	<t:form id="ilAddDdmx" dialog="true" caption="新增" submit="NOT">
		<t:fieldLayout prefix="add">
			...
			<t:fieldCalendar property="jiaofrq" caption="交付时间" width="150" format="yyyy-MM-dd HH:mm" notNull="true"></t:fieldCalendar>
...
<script type="text/javascript">
	...
	function func_button_copy() {
		...
		$.sdcui.ajaxUtil.ajax({
			...
			data : "jiaofrq=" + (record[0].jiaofrq).substr(0,10) ,
   ...
   function getGyzq(){
		...
		if(typeof jiaofrq!='undefined'&&jiaofrq!=''){
				...
				data : "jiaofrq=" +jiaofrq.substr(0,10) ,
				success : function(result) {
	...
	function cangkdmValidate() {
		...
		var ghlx =nedghlx.indexOf('P')==-1&&nedghlx.indexOf('V')==-1?'N'+gonghlx.substr(1):nedghlx;
		var gongysdm = $("#il_gongysdm").fieldValue();
	...
	function func_button_edit() {
		...
		for ( var i = 0; i < param.length; i++) {
			...
			params.push('ddmx[' + i + '].xiehzt=' + param[i].xiehzt);
	...
	function func_button_delete() {
	 ...
	 if("" == $('#il_editor').fieldValue()){
	  $('#button_uneffect').hide();
	  $('#button_effect').hide();
	 }
