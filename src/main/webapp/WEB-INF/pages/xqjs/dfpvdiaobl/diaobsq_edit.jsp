	...
	*/
	...
	$(document).ready(function(){
		$("#sqedit_lingjbh").bind("focusout",function(){
		      ...
		      if(lingjbh!=""){
		           $.sdcui.ajaxUtil.ajax({
		 	 	   url:"${ctx}/dfpvdiaobl/getlingjmc.ajax?"+date,
	...
	function  func_reload(bancstr,zhuangt){
		 ...
		 * form跳转模式提交
		 */
		 window.showModalDialog ("${ctx}/dfpvdiaobl/initEdit.do?removeId=layout-north,layout-west,layout-south&usercenter="+usercenter+"&diaobsqdh="+diaobsqdh+"&diaobsqsj="+diaobsqsj+"&banc="+bancstr+"&zhuangt="+zhuangt,
		window,
	...
	function  func_button_edit(){
		   ...
		   //修改ajax请求       
		   $.sdcui.ajaxUtil.ajax({
	         url:"${ctx}/dfpvdiaobl/update.ajax",
	 	 	   data:param.join('&'),
	 	       success:function (result){
	 	        if(result.flag != null){
	 	        	alert(result.flag);
	...
	function  func_button_delete(){
		   ...
		   //修改ajax请求       
		   $.sdcui.ajaxUtil.ajax({
	         url:"${ctx}/dfpvdiaobl/delete.ajax",
	 	 	   data:param.join('&'),
	 	       success:function (result){
	 	    		if(result.flag != null){
		 	        	alert(result.flag);
		 	        	   ...
		 	        	   var  select=$('#grid_diaobsq').grid('getSelectedRecords');
		 	      			if(red.length==select.length){//如果删除全部数据，则关闭此窗口
		 		 	    		...
		 		 	    		//父窗口刷新  window.dialogArguments.location.reload()无法使用
		 		 	    		 window.dialogArguments.location.href='${ctx}/dfpvdiaobl/initSq.do?dirName=调拨令查询-DFPV调拨令-执行层';
		 		 		         window.close();
		 	 	    	       }
		 	               }
	 	    	}
