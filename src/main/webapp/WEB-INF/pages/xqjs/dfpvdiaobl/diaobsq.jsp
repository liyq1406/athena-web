			...
			<table align="center" width="100%">
				...
				<tr>
				   <td width="100%" colspan="2">
				    <t:fieldLayout id="dbsq" prefix="dbsq" columns="3">
				   		 	<t:fieldSelect property="center" caption="${i18n.usercenter}" width="110">
				   		  		<t:fieldOption value="UL" text="UL"></t:fieldOption>
				   		 	</t:fieldSelect>
							<t:fieldText property="huijkm" caption="${i18n.huijkm}"  maxLength="10" width="150"></t:fieldText>
				...
				<tr>
					<td colspan="2"><t:grid id="grid_diaobsq"  editable="true" idKeys="lingjbh" usePager="false" submit="NOT" reset="NOT" save="NOT" insertAlias="diaobsqmxs" 
							...
							<t:gridCol property="lux"  editor="fieldText" type="text"  caption="${i18n.lux}" editorOptions="{'notNull':true,'expression':'^[A-Za-z0-9]{3}$','expressionMessage':'制造路线为2位数字1位字母或三位字母组成'}"  ></t:gridCol>
							<t:gridCol editor="fieldText" property="shenbsl" type="text" caption="${i18n.shenbsl}" editorOptions="{'notNull':true,'expression':'^[0-9]{1,8}(?:\\.[0-9]{1,3})?$|^0\\.[0-9]{1,3}$','expressionMessage':'整数部分最大位8位，小数部分最大位3位'}"></t:gridCol>
...
<script type="text/javascript">
        ...
        var usercenter = $('#dbsq_center').fieldValue();
		     ...
		     // params.push('lux='+lux);
		      ...
		      var date = params.join('&');
		    	  ...
		    	  $.sdcui.ajaxUtil.ajax({
			 	 	   url:"../dfpvdiaobl/getlingjmc.ajax?"+date,
			 	 	   //data:params.join('&'),
	...
	function func_set_lux() {
        ...
        var usercenter = $('#dbsq_center').fieldValue();
		      ...
		      var date = params.join('&');
		    	  $.sdcui.ajaxUtil.ajax({
			 	 	   url:"../dfpvdiaobl/querylucz.ajax?"+date,
			 	       success:function (result){
			 	        if(result.flag != null){
			 	        	alert(result.flag);
			 	        	$(cell).attr("title",result.flag).addClass("validate-error");
      ...
      function validate(lingjbh,shenbsl,yaohsj){
          	...
          	$.sdcui.pageUtils.goPage({
		        ...
		        function func_button_sqsubmit(){
                     ...
                     if(!$('#form_diaobsq').form('validate')){
	                	...
	                	var leix = $('#dbsq_leix').fieldValue();
	                	params.push('leix='+leix);
	                		                	
	                	if("UL" != usercenter && leix=="1" )
	                	{
		                 	alert("该用户中心下暂时不支持DFPV类型的零件");
		                 	return false;
	                	}
		                	  ...
		                	  $.sdcui.ajaxUtil.ajax({
	 	 	                         url:"${ctx}/dfpvdiaobl/insert.ajax",
	 	 	                         data:params.join('&'),
	 	                             success:function (result){
	 	                                   ...
	 	                                   var data = param.join('&');
	 	                                 $.sdcui.ajaxUtil.ajax({
	 		 	 	                         url:"${ctx}/dfpvdiaobl/print.ajax?"+data,
	 		 	 	                         //data:param.join('&'),
	 		 	                             success:function (result){
		...
		function func_button_importExcel()
		{
			...
			if (fvalue.length > 0) 
			{
                ...
                //会计科目
                var huijkm = $('#dbsq_huijkm').fieldValue();
                form.action = '${ctx}/dfpvdiaobl/upload.do?usercenter='+ usercenter + "&chengbzx=" + chengbzx + "&huijkm=" + huijkm+"&beiz=" + beiz;
				form.submit();
		...
		function callbackdow(msg,name) 
		{
			alert(msg);
			$.sdcui.pageUtils.goPage({
				url : '../dfpvdiaobl/errdownLoad.do?name='+name,
				mode : 'form',
				beforeSubmit : function(){
					return true;
				}
