...
<%request.setAttribute("pageId","410");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_410}</title>
...
$(function(){
	$('#form_user_${pageId}').form({
		onreload:true,
 		fields:[
                {fieldType:'fieldText',property:'ckandzick',caption:'${i18n.zickbh}',dataType:'letter_numeric',limitedLength:'3',limitedLength2:'6',value:st_Cangkbh,notNull:true},
                ...
                {fieldType:'fieldSelect',property:'zhuangtsx',caption:'${i18n.zhuangtsx}',notNull:false,includeNull:true,src:'<%=request.getContextPath()%>/common/zhuangtsx410.do'},           
                {fieldType:'fieldSelect',property:'zhijz',caption:'${i18n.zhijz}',notNull:false,src:'<%=request.getContextPath()%>/common/zhijz.do'}
	 			]
 		});
	
	$('#grid_user_${pageId}').grid({
		keyFields:['ckandzick','ush','lingjbh','lingjmc','lingjsl','elh','pich','zhijz','gongysdm','gongysmc','blh','kuwbh','uth','shixq','flag','danw'],
		id:'grid_user_${pageId}',
		showCheckbox:true,
		//翻页查询交易
		src:'<%=request.getContextPath()%>/yichang/41001.do',
		cols:[
									 ...
									 showPromptMsg('red','${i18n.C_C_cuowckh}')
									  return;
										...
										$('#grid_user_${pageId}').grid("setQueryParams", params);
								 		...
								 		var url = "<%=request.getContextPath()%>/yichang/41001.do";
								 		g_isSub=0;
					...
					//全选
	 				...
	 				{name:'submit',caption:'${i18n.ok}[F3]',keyboard:'F3',action:function(){
						...
						params = params.concat(records);
				 		var url = "<%=request.getContextPath()%>/yichang/41002.do";
						$('#form_user_${pageId}').form("submitUrl", params, url, function(results){
							 if (showPromptRequestMsg(results)==true){
...
<table class="ui-grid" cellspacing="1" width="100%">
      <tr>
		...
		<td class="field-label">${i18n.zhuangtsx}：</td>
