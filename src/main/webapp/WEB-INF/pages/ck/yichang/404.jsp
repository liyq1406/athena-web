...
$(function(){
	$('#form_user_${pageId}').form({
		onreload:true,
 		fields:[
				...
				//{fieldType:'fieldText',property:'gongfsl',dataType:'dbnumber',isPlus:true,caption:'${i18n.gongfsl}',notNull:false},
				//{fieldType:'fieldText',property:'liaofsl',dataType:'dbnumber',caption:'${i18n.liaofsl}',notNull:false},
				{fieldType:'fieldSelect',property:'flag',onselect:'flagchange()',caption:'${i18n.gonglf}',internation:true,notNull:false,src:'<%=request.getContextPath()%>/common/zhijzl.do'},
				...
				{fieldType:'fieldText',property:'shuifsl',dataType:'dbnumber',caption:'${i18n.shuifsl}',notNull:false},
	...
	$('#grid_user2_${pageId}').grid({
		...
		{property:'hegsl',caption:'${i18n.hegsl}'},
		{property:'elh',caption:'${i18n.elh}'},
		{property:'pich',caption:'${i18n.pich}'},
		{property:'gongfsl',caption:'${i18n.gongfsl}'},
		{property:'liaofsl',caption:'${i18n.liaofsl}'},
		{property:'shuifsl',caption:'${i18n.shuifsl}'}
  		]
	});
	//F1查询 
 	$('#btn_search').bind('click',function(){ 
		...
		$('#xiaohdh input').val(xiaohd);
 		...
 		var url = "<%=request.getContextPath()%>/yichang/40401.do";
			 ...
			 if (showPromptRequestMsg(results)==true){
				 ...
				 if (results.result.response == "0001"){
					//加载返回数据
						...
						$('#lingjsl span').html(results.result.parameter.shenbsl)
	...
	$('#grid_user_${pageId}').grid({
		...
		showCheckbox:true,
		src:'<%=request.getContextPath()%>/yichang/40401.do',
		cols:[
  		...
  		buttons:[
 				...
 				{name:'delete',caption:'${i18n.delete}[F6]',keyboard:'F6',action:function(){
 					...
 					params.push("shengbd="+$('#shengbd span').html());
 			 		var url = "<%=request.getContextPath()%>/yichang/40405.do";
 					 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
 						 if (showPromptRequestMsg(results)==true){
 							 //加载返回数据
 							 $('#grid_user_${pageId}').grid('clearRecords');
	...
	//根据输入随废UC号带出 ,随废零件编号,随废零件名称
 	$('#suifuch').bind('enter',function (){
 		...
 		params.push("zhengclsh="+zhengclsh);
			var url = "<%=request.getContextPath()%>/yichang/40404.do";
			 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
				 if (showPromptRequestMsg(results)==true){
					//赋值
					if (results.result.response == "0000"){
	...
	//F3随废增加
 	$('#btn_add').bind('click',function(){
 		...
 		var url = "<%=request.getContextPath()%>/yichang/40403.do";
 		 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
			 if (showPromptRequestMsg(results)==true){
 	...
 	$('#btn_ok').bind('click',function(){
		...
		if("4" == zerzt){
 		...
 		var url = "<%=request.getContextPath()%>/yichang/40402.do";
		 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
			 if (showPromptRequestMsg(results)==true){
				 if (results.result.response == "0000"){
					 row = results.result.parameter;
				     if (row && row != null){
		 					$('#grid_user2_${pageId}').grid("addRow",row);
			 	    		return false;
		 				}
					 $('#form_user_${pageId}').form('resetFields');
					 $('#grid_user_${pageId}').grid('clearRecords');
...
function flagchange1(){
		...
		$('#zerztdm').fieldSelect('clearSelect');
			...
			if(chengysbh.length>0){
			 ...
			 $('#zerztdm').fieldSelect('clearSelect');//0011355
		     $('#zerztdm input').val("")
			 $('#zerztmc input').val("")
		     $('#zerztdm').fieldSelect('resetUrl','<%=request.getContextPath()%>/yichang/zerztdm.do?zerzt='+zerzt);
		 	 $('#zerztdm input').attr("readonly",false);
