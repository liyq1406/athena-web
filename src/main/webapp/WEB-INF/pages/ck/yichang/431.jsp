...
<%request.setAttribute("pageId","431");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_431}</title>
...
$(function(){
	$('#form_user_${pageId}').form({
		onreload:true,
 		fields:[
                {fieldType:'fieldSelect',property:'ckandzick',caption:'${i18n.zickbh}',notNull:true,src:'<%=request.getContextPath()%>/common/wareHouseck.do'+'?buhgpk='+4+'&buhgpk2='+0},
	...
	$('#grid_user_${pageId}').grid({
		...
		//checkedOnlyOne:true,
		//翻页查询交易
		src:'<%=request.getContextPath()%>/yichang/43101.do',
		editSubmitSrc:'<%=request.getContextPath()%>/yichang/43102.do',
		cols:[
	   	  			...
	   	  			{name:'btn_search',caption:'${i18n.search}[F1]',keyboard:'F1',action:function(){
			   	  					 ...
			   	  					 if(id==0){
			   	  						 ...
			   	  						 showPromptMsg('red','${i18n.C_C_cuowckh}')
			   	  						  return;
			   	  							...
			   	  							params.push("cangklx=4");
			   	  							$('#grid_user_${pageId}').grid("setQueryParams", params);
			   	  					 		var url = "<%=request.getContextPath()%>/yichang/43101.do";
			   	  					 		g_isSub=0;
	   	 				...
	   	 				{name:'btn_ok',caption:'${i18n.ok}[F3]',keyboard:'F3',action:function(){
		   	 				...
		   	 				//验证Grid的输入格式
		   	 		 		...
		   	 		 		params.push("lingjh="+$('#lingjh input').val());
		   	 		 		params.push("cangklx=4");
		   	 		 		paramsQuery = jQuery.extend(true,[],params);
		   	 		 		var url = "<%=request.getContextPath()%>/yichang/43103.do";
		   	 				 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
		   	 					 if (showPromptRequestMsg(results)==true){
		   	 						//加载grid
		   	 							$('#grid_user_${pageId}').grid("parseResult",results)
