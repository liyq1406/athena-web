...
<%request.setAttribute("pageId","617");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_617}</title>
	...
	$('#form_user_${pageId}').form({
		onreload:true,
 		fields:[
                ...
                {fieldType:'fieldSelect',property:'zhuangt',caption:'${i18n.zhuangt}',internation:true,notNull:false,includeNull:true,src:'<%=request.getContextPath()%>/common/diaobzxzt.do'},
  		...
  		buttons:[
				...
				{name:'btn_search',caption:'${i18n.search}[F1]',keyboard:'F1',action:function(){
 				...
 				{name:'info',caption:'${i18n.info}[F2]',keyboard:'F2',action:function(){
                    ...
                    if(currPandfs.length < 1){
 					...
 					var url = "<%=request.getContextPath()%>/rongqi/61702.do";
 					 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
 						 if (showPromptRequestMsg(results)==true){
 							 if (results.result.response == "0000"){
 									...
 									$("#putform").attr("action","<%=request.getContextPath()%>/rongqi/6170101.do"+st_Params);
				...
				{name:'btn_chexiao',caption:'${i18n.repeal}[F3]',keyboard:'F3',action:function(){
			 		if($('#zhuangt input').val()=='已完成'){
			 			showPromptMsg('red','${i18n.selectyiwanc}')
						return false;
			 		}
					...
					if(confirm($.sdcui.resource.i18n['C_remove'])){
			 		 //提交数据
			 		var params = $('#grid_user_${pageId}').grid('getSelectedAll');
			 		var paramss = $('#form_user_${pageId}').form("getFormParam");
			 		paramsQuery = jQuery.extend(true,[],params);
					params.push("currentPage="+currentPage);
					for(var i=0;i<paramss.length;i++)
						{
						 params.push(paramss[i]);
						}
			 		var url = "<%=request.getContextPath()%>/rongqi/61703.do";
					 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
						 if (showPromptRequestMsg(results)==true){
							//加载grid
								$('#grid_user_${pageId}').grid("parseResult",results)
