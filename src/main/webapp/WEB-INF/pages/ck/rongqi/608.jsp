...
<%request.setAttribute("pageId","608");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_608}</title>
		...
		{property:'create_time',caption:'${i18n.baofsqdsj}'},
  		...
  		buttons:[
				...
				{name:'info',caption:'${i18n.info}[F2]',keyboard:'F2',action:function(){
					...
					var url = "<%=request.getContextPath()%>/rongqi/60802.do";
					 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
						 if (showPromptRequestMsg(results)==true){
							 if (results.result.response == "0000"){
									...
									$("#putform").attr("action","<%=request.getContextPath()%>/rongqi/6080101.do"+st_Params);
				...
				{name:'btn_chexiao',caption:'${i18n.repeal}[F3]',keyboard:'F3',action:function(){
					...
					params.push("currentPage="+currentPage);
			 		var url = "<%=request.getContextPath()%>/rongqi/60803.do";
					 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
						 if (showPromptRequestMsg(results)==true){
							 $('#grid_user_${pageId}').grid("parseResult",results)
						 }
