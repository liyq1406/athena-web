...
<%request.setAttribute("pageId","605");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_605}</title>
...
$(function(){
	...
	$('#grid_user_${pageId}').grid({
		...
		{property:'danjbh',caption:'${i18n.danjbh}'},
  		...
  		{property:'wuld',caption:'${i18n.diaochuwld}'},
  		...
  		buttons:[
				...
				,{name:'info',caption:'${i18n.info}[F2]',keyboard:'F2',action:function(){
					...
					var url = "<%=request.getContextPath()%>/rongqi/60502.do";
					 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
						 if (showPromptRequestMsg(results)==true){
							 if (results.result.response == "0000"){
									...
									$("#putform").attr("action","<%=request.getContextPath()%>/rongqi/6050101.do"+st_Params);
				...
				{name:'btn_chexiao',caption:'${i18n.repeal}[F3]',keyboard:'F3',action:function(){
					...
					params.push("currentPage="+currentPage);
			 		var url = "<%=request.getContextPath()%>/rongqi/60503.do";
					 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
						 if (showPromptRequestMsg(results)==true){
							 $('#grid_user_${pageId}').grid("parseResult",results)
