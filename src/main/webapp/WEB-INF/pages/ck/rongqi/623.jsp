...
<%request.setAttribute("pageId","623");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_623}</title>
...
$(function(){
	...
	$('#grid_user_${pageId}').grid({
		...
		showCheckbox:true,
		//翻页查询交易
		src:'<%=request.getContextPath()%>/rongqi/62301.do',
		cols:[
 	...
 	$('#info').bind('click',function(){
        ...
        if(currPandfs.length  < 1){
         ...
         var params = $('#grid_user_${pageId}').grid('getSelectedAll');
			...
			var url = "<%=request.getContextPath()%>/rongqi/62302.do";
			 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
				 if (showPromptRequestMsg(results)==true){
					 if (results.result.response == "0000"){
						...
						$("#putform").attr("action","<%=request.getContextPath()%>/rongqi/6230101.do"+st_Params);
	...
	//F3撤销
 	$('#btn_chexiao').bind('click',function(){
		...
		params.push("gongysdm="+$('#gongysdm input').val());
 		...
 		var url = "<%=request.getContextPath()%>/rongqi/62303.do";
		 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
			 if (showPromptRequestMsg(results)==true){
				 $('#grid_user_${pageId}').grid("parseResult",results)
			 }
