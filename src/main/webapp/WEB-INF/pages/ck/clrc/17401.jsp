...
<%request.setAttribute("pageId","17401");%>
<%
Map  map = (Map)request.getAttribute("result");

int pagesize=1;

String rongqfhdh =(String)map.get("rongqfhdh");

int cs= Integer.parseInt(map.get("cishu").toString());//原次数

int cishu=cs+1;//当前次数 0011242

%>

<%request.setAttribute("cishu",cishu);%>
<%request.setAttribute("rongqfhdh",rongqfhdh);%>

<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_17401}</title>
...
function printWithAlert() {//打印方法
    	...
    	var djbh = '${rongqfhdh}';  
    	params.push('rongqfhdh='+djbh);  
    	    	  	
    	params.push('cishu='+${cishu});   
    	
    	$.ajax({
			type:'post',
			url:'<%=request.getContextPath()%>/clrc/17403.do',
			data:params.join('&'),
			type:"POST",
			dataType : 'json',
...
<td align="left" id="rongqfhdh"><%=map.get("rongqfhdh")%></td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<%	
...
<tr>
<% if(i ==0 ){ %>
<td align="center" colspan="5"><h3>空出出厂单(补打<%=cishu%>)-(一式三联   供应商/承运商联)</h3></td>
<%}else{ %>
<td align="center" colspan="5"><h3>空车出厂单(补打<%=cishu%>)-(一式三联   门卫联)</h3></td>
<% }%>
