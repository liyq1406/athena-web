		 	...
		 	buttons:[
				...
				,{name:'submit',caption:'${i18n.ok}[F3]',keyboard:'F3',action:function(){
					//获取参数
			 		...
			 		var url = "<%=request.getContextPath()%>/kunei/12104.do";
			 		$('#form_user_${pageId}').form("submitUrl", params, url, function(results){
			 			 if (showPromptRequestMsg(results)==true){
			 			 	st_BackCount = 3;
			 				if (results.result.response=="0000" && !results.result.respdesc){
				 				//操作成功--返回
				 				window.location.href = "<%=request.getContextPath()%>/kunei/121.do"+st_Params+"&st_backCount="+st_BackCount;
			 				}
						 }
					});
				}}
