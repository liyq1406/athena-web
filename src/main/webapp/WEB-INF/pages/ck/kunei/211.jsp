...
<%request.setAttribute("pageId","211");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_211}</title>
		...
		$(function(){
		 	...
		 	$('#lingjbh').bind('enter',function(){
				...
				//页面数据提交并跳转
		 		if ($('#shifcj input').val()=="Y"){  //创建US并登记盘点
		 			var url = "<%=request.getContextPath()%>/kunei/21102.do";
			 		$('#form_user_${pageId}').form("submitUrl", params, url, function(results){
			 			 if (showPromptRequestMsg(results)==true){
							...
							$("#putform").attr("action","<%=request.getContextPath()%>/kunei/21205.do"+st_Params);
			...
			$('#ush').bind('enter',function(){
				...
				$('#serachKey').val(params);
				//页面数据提交并跳转
				var url = "<%=request.getContextPath()%>/kunei/21104.do";
		 		$('#form_user_${pageId}').form("submitUrl", params, url, function(results){
		 			 if (showPromptRequestMsg(results)==true){
		 			 	if(results.result.respdesc&&results.result.respdesc.length>0){
		 			 		//打开提示框
						...
						var strJSON=JSON.stringify(results);
						$('#putvalue').val(strJSON);
						$("#putform").attr("action","<%=request.getContextPath()%>/kunei/21206.do"+st_Params);
		    			document.putform.submit();
					 }
				});
			});
			...
			if ( $('#serachKey').val()!=""){
				...
				for(i=0;i<params.length;i++){
					...
					if (vParam[0].indexOf("renwbh") != -1){                   //任务编号
						...
						continue;
					} else if (vParam[0].indexOf("cangkbh") != -1){            	  //US号
