...
<%request.setAttribute("pageId","121");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_121}</title>
		...
		$(function(){
			...
			$('#form_info_${pageId}').form({
				...
				{fieldType:'fieldText',property:'shijkcsl',dataType:'dbnumber',caption:'${i18n.shijkcsl}',notNull:false},
			...
			$('#grid_user_${pageId}').grid({
				keyFields:['ush','usxh','lingjsl','ucrl','jiufhusxh','jiufhussl','jiufhucrl','str2'],
				id:'grid_user_${pageId}',
				showCheckbox:false,
				//翻页查询交易
				src:'<%=request.getContextPath()%>/kunei/12101.do',
				cols:[
				{property:'ush',caption:'${i18n.ush}'},
		  		...
		  		{property:'jiufhusxh',caption:'${i18n.jiuf}${i18n.usrq}',bind:'jiufhusxh',dataType:'letter_numeric',limitedLength:'5',notNull:true,editor:'fieldText'},
		  		{property:'jiufhussl',caption:'${i18n.jiufhussl}',bind:'jiufhussl',dataType:'dbnumber',notNull:true,editor:'fieldText'},
		  		{property:'jiufhucrl',caption:'${i18n.jiuf}${i18n.uchl}',bind:'jiufhucrl',dataType:'dbnumber',notNull:true,editor:'fieldText'}
			]});
			...
			//F1查询事件
		 	$('#btn_search').bind('click',function(){
				...
				$('#grid_user_${pageId}').grid("setQueryParams", params);
		 		var url = "<%=request.getContextPath()%>/kunei/12101.do";
		 		$('#form_user_${pageId}').form("submitUrl", params, url, function(results){
		 	...
		 	$('#btn_save').bind('click',function(){
				...
				if ($('#shijljbh input').val()!=""){ 
		 		...
		 		var url = "<%=request.getContextPath()%>/kunei/12102.do";
		 		$('#form_user_${pageId}').form("submitUrl", params, url, function(results){
		 			if (showPromptRequestMsg(results)==true){
						 //赋值
						 $('#elh span').html(results.result.parameter.elh);
						 $('#lingjbh span').html(results.result.parameter.lingjbh);
						 $('#lilkcsl span').html(results.result.parameter.lilkcsl);
						 $('#gongysdm span').val(results.result.parameter.gongysdm);
						 $('#gongysdm input').val(results.result.parameter.gongysdm);
						 $('#uth span').html(results.result.parameter.uth);
						 $('#rukrq span').html(results.result.parameter.rukrq);
						 $('#jiufdh span').html(results.result.parameter.jiufdh);
		 	...
		 	$('#btn_sumbit').bind('click',function(){
				...
				if ($('#shijljbh input').val()!=""){ 
		 		...
		 		var records = $('#grid_user_${pageId}').grid('getValueChangedAll');
		 		params = params.concat(records);
		 		var url = "<%=request.getContextPath()%>/kunei/12103.do";
		 		$('#form_user_${pageId}').form("submitUrl", params, url, function(results){
		 			if (showPromptRequestMsg(results)==true){
						 //判断是否要改包装 (1、需要改包装   0、不需要改包装 )
						 if (results.result.parameter.caozzt == "1"){   //需要改包装
						 	//存放查询条件
							...
							$("#putform").attr("action","<%=request.getContextPath()%>/kunei/P12101.do"+st_Params);
		...
		$('#str2 input').attr("disabled",true);
			if ( $('#serachKey').val()!=""){
				...
				for(i=0;i<params.length;i++){
					...
					if (vParam[0].indexOf("jiufdh") != -1){                 //纠纷单号
						$('#jiufdh input').val(vParam[1]);
						continue;
					} else if (vParam[0].indexOf("elh") != -1){       		//EL号
	 			...
	 			<table class="ui-grid" cellspacing="1" width="100%">
					<tbody>
						...
						<tr>
							...
							<td>${i18n.jiufyy}:</td>
							<td><div class="ui-field" id="jiufyySel"></div><div class="ui-field" id="jiufyy"></div></td>
							<td></td>
							<td></td>
						</tr>
						<tr>
