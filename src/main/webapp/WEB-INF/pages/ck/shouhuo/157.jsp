...
<%request.setAttribute("pageId","157");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_157}</title>
		...
		<script type="text/javascript">
			$(function(){
				...
				$('#form_user_${pageId}').form({
					...
					{fieldType:'fieldLabel' , property:'xiehd',caption:'${i18n.xiehzt}'}
					]
				}); 
                // grid
               $('#grid_user_${pageId}').grid({
					id:'grid_user_${pageId}',
					src:'<%=request.getContextPath()%>/shouhuo/15701.do',
					cols:[
				...
				buttons:[
					    ...
					    if(!$('#form_user_${pageId}').form("validateParam")){
						  ...
						  $('#grid_user_${pageId}').grid("setQueryParams", params);
						   var  url = "<%=request.getContextPath()%>/shouhuo/15701.do";
						   $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
								 if (showPromptRequestMsg(results)==true){
									...
									$('#xiehd span').html(results.result.parameter.xiehd);
								 }
						 });
				    		return false;  	 
					}}
