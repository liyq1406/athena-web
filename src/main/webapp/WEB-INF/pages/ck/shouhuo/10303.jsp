...
<%request.setAttribute("pageId","10303");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_10303 }</title>
			...
			$(function(){
				$('#form_user_${pageId}').form({
					fields:[
						...
						//rencong 0011752
						{fieldType:'fieldText',property:'suifljmc',dataType:'laiy',caption:'${i18n.lingjmc}',limitedLength:'40'},
				 ...
				 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
				 			if (showPromptRequestMsg(results)==true){
				 		          //if(results.result.response != 'C_1403'){	
									...
									var strJSON=JSON.stringify(results);
									$('#putvalue').val(strJSON);
									$("#putform").attr("action","<%=request.getContextPath()%>/shouhuo/P10304.do"+st_Params+'&'+ params.join('&')+'&'+temp.join('&'));
		    			    		document.putform.submit();
		    			    	 //}
							 }
				 	});
