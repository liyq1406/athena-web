...
<%request.setAttribute("pageId","10304");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_10304 }</title>
			...
			$(function(){
				$('#form_user_${pageId}').form({
					fields:[
                        ...
                        {fieldType:'fieldSelect',property:'jus',biaos:'enter',src:'<%=request.getContextPath()%>/common/jusyy.do',caption:'${i18n.jusyy}'},
						...
						{fieldType:'fieldLabelInput',property:'caozy',dataType:'laiy',value:'${userName}'},
						{fieldType:'fieldText',property:'yansy',notNull:true,dataType:'laiy',maxLength:'10',caption:'${i18n.yansy}',value:'${userName}'},
			   ...
			   $("#jus .value").bind("keydown",function(event){
			 		if(event.keyCode == 13){
			 			...
			 			/*		 					 		
				 		if(v=="CW"){
				 	     $('#shijljbh input').attr("readonly",false);
				 	     $('#shijljbh input').attr("disabled",false);
				 		}
				 		*/
			       }
				...
				$('#btn_submit').bind('click',function(){
			        ...
			        $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
				     if (showPromptRequestMsg(results)==true){
						...
						//$("#putform").attr("action","<%=request.getContextPath()%>/shouhuo/P10303.do"+st_Params+'&'+temp.join('&'));
						$("#putform").attr("action","<%=request.getContextPath()%>/shouhuo/P10302.do"+st_Params+'&'+temp.join('&'));
					 ...
					 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
					 			if (showPromptRequestMsg(results)==true){
					 		          if(results.result.response != 'C_1403'){	
										...
										var strJSON=JSON.stringify(results);
										$('#putvalue').val(strJSON);
										$("#putform").attr("action","<%=request.getContextPath()%>/shouhuo/P10303.do"+st_Params+'&'+ params.join('&')+'&'+temp.join('&'));
			    			    		document.putform.submit();
			    			    	 }
								 }
					 	});
		    ...
		    $(document).ready(function(){
		    	var parameter = ${result.result.parameter};
					...
					$('#shixrq input').val(parameter.shixrq);
				 	...
				 	//$('#shijljbh input').attr("readonly",true);
				 	//$('#shijljbh input').attr("disabled",true);
