...
<%request.setAttribute("pageId","10302");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_10302 }</title>
			...
			$(function(){
				...
				buttons:[
					...
					,{name:'dayysd',caption:'${i18n.dayysd}[F8]',keyboard:'F8',action:function(){
							var params =[];
					     	params.push("uth=" + $('#uth span').html());
							var url="<%=request.getContextPath()%>/shouhuo/10307.do";
						    $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
					 		 if (showPromptRequestMsg(results)==true){ 
						 		     $('#grid_user_${pageId}').grid('clearRecords');
									 $('#form_user_${pageId}').form('clearValue');
									 window.location.href = '<%=request.getContextPath()%>/shouhuo/103.do'+st_Params;
									 	return true;
								 }
							});
					}}
				  ,{name:'back',caption:'${i18n.back}[F11]',keyboard:'F11',action:function(){
						window.location.href = '<%=request.getContextPath()%>/shouhuo/103.do'+st_Params+'&'+temp.join('&');
					 	return true;
					}}
