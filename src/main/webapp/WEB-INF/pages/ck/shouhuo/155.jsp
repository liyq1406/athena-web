...
<%request.setAttribute("pageId","155");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_155}</title>
		...
		<script type="text/javascript">
			$(function(){
				...
				$('#form_user_${pageId}').form({
					...
					{fieldType:'fieldSelect' ,property:'xiehd',caption:'${i18n.xiehzt}',includeNull:true,src:'<%=request.getContextPath()%>/common/zickxhzt.do?cangkbh='+st_Cangkbh},
               ...
               $('#grid_user_${pageId}').grid({
					...
					src:'<%=request.getContextPath()%>/shouhuo/15501.do',
					cols:[
				...
				,buttons:[
					    ...
					    if(!$('#form_user_${pageId}').form("validateParam")){
						  ...
						  $('#grid_user_${pageId}').grid("setQueryParams", params);
						   var  url = "<%=request.getContextPath()%>/shouhuo/15501.do";
		 ...
		 //加载时判断是否是回退
			$(document).ready(function(){
				if ( $('#serachKey').val()!=""){
					...
					for(i=0;i<params.length;i++){
						var vParam = params[i].split("=");
						if (vParam[0].indexOf("xiehd") != -1){                   //blh
							$('#xiehd input').val(vParam[1])
							$('#xiehd input').attr("realValue",vParam[1])
							continue;
						}
					}
					//设置分页条件
