...
<%request.setAttribute("pageId","17703");%>
<!-- V4_022 -->
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_17703 }</title>
				 ...
				 $('#form_user_${pageId}').form("submitUrl", params, url, function(results){
				 			if (showPromptRequestMsg(results)==true){
				 		          //if(results.result.response != 'C_1403'){	
									...
									var strJSON=JSON.stringify(results);
									$('#putvalue').val(strJSON);
									$("#putform").attr("action","<%=request.getContextPath()%>/shouhuo/P17704.do"+st_Params+'&'+ params.join('&')+'&'+temp.join('&'));
		    			    		document.putform.submit();
		    			    	 //}
							 }
				 	});
