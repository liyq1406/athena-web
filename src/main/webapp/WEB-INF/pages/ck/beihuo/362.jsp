...
<%request.setAttribute("pageId","362");%>
<t:html i18n="i18n.ck">
	<head>
		<title>${i18n.title_362}</title>
...
$(function(){
	...
	$('#form_user_${pageId}').form({
	    onreload:true,
 		fields:[
               //huxy_11470 {fieldType:'fieldText',property:'lingjbh',dataType:'letter_numeric',limitedLength:'10',caption:'${i18n.lingjbh}',notNull:true},
                {fieldType:'fieldSelect',property:'cangkbh',caption:'${i18n.cangk}',notNull:true,src:'<%=request.getContextPath()%>/common/wareHousecpck.do'}
	...
	$('#grid_user1_${pageId}').grid({
		keyFields:['cangkbh','lingjbh','lingjmc','shenbsl','magsl','zaitsl','gaibzsl','zhijsl','kucsl','jiessl','fanxsl',
		            'tuigyssl','dengdjfsl','jussl','zhuanbhgsl','buhgsl','fangyssl','daixhsl','yixhsl','kehsl','shul'],
		id:'grid_user1_${pageId}',
		//翻页查询交易
		src:'<%=request.getContextPath()%>/beihuo/36201.do',
		//pageSize : 5,
		cols:[
	...
	$('#grid_user2_${pageId}').grid({
		...
		cols:[
			...
			{property:'shul',caption:'${i18n.huizsl}',width:50}
	          ...
	          $('#btn_search').bind('click',function(){
                ...
                $('#btn_print').bind('click',function(){
				    var params = $('#form_user_${pageId}').form("getFormParam");
					var records = $('#grid_user1_${pageId}').grid('getRecordAll');	
					//if(records.length <3){
                    //       showPromptMsg('red','${i18n.selectoneerror}');
                    //      return false;
                    // }
                     params=params.concat(records);
					 var url = "<%=request.getContextPath()%>/beihuo/36202.do";
         ...
         $("#baozzt").bind('enter',function(event){
	     	    ...
	     	    var baozzt = parseInt($('#baozzt input').val())+1 ;
                //判断包装状态是否有误 
                if(baozzt-1 < 2 || baozzt-1 > 19 ||  baozzt-1 == 13){
                ...
                if(p[baozzt].split("=")[1] == 0){
	             ...
	             var params =[];
	             var realBaozzt = baozzt-1;
	             params.push("baozzt="+ realBaozzt);
   ...
   <tr>
      ...
      <td width="10%"></td>
       <td align="left" width="20%"> <div><span id="btn_search" class="ui-button">${i18n.search}[F1]</span>
       			 <!--huxy_11464  -->
                 <span id="btn_print" class="ui-button">${i18n.print}[F2]</span>
