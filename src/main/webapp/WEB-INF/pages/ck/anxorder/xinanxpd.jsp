...
<t:html title="${i18n.xbkcpd}" ><!-- 新按需盘点 -->
<head>
...
<t:form id="form_chushzy" submit="NOT" reset="NOT">
  <!--查询列表开始-->
  <t:grid id="chushzyGridQuery" caption="${i18n.pdcx}" idKeys="lingjbh,usercenter,shengcxbh,xiaohd,mos" showNum="true" 
	src="/xinanxpd/queryXinAnxpd.ajax"  edit="NOT" load="false" remove="NOT"  add="NOT"  showCheckbox="true" scrollHeight="135">
	...
	<t:gridCol property="xiaohl" caption="SPPV消耗量"></t:gridCol>
	<t:gridCol property="yingyl" caption="盈余量"></t:gridCol>
	...
	<form id="exp" name="exp" action="${ctx}/xinanxpd/downLoadFilePd.do" method="post" style="background-color: white;" target="hiddenUp">
...
function func_button_save(){
	...
	//空格查询
	function grid_chushzyGridQuery_beforeSubmit() {
		$("#layout_usercenter").fieldValue(trim($("#layout_usercenter").fieldValue()));
		$("#layout_shengcxbh").fieldValue(trim($("#layout_shengcxbh").fieldValue()));
		$("#layout_lingjbh").fieldValue(trim($("#layout_lingjbh").fieldValue()));
		$("#layout_xiaohd").fieldValue(trim($("#layout_xiaohd").fieldValue()));
	}
     ...
     function jscy(){
		...
		var pandtime = rowElement.find('td:eq(' + pandtimeIndex + ')').text();
     	if (daixh == "" ||      daijf == "" || xiaohl == "" || yingyl == "")
     	{//xianbllkc == "" ||  chukl == "" ||  yicdxh == "" ||
     		rowElement.find('td:eq(' + chayIndex + ')').text("");
     		alert('${i18n.jslshbc}');
     	} 
     	else 
     	{
     		...
     		var flagYingyl = parseFloat(yingyl);//flagXianbllkc +    flagChukl +    							    		
            //flagDaixh +    flagXiaohl + flagYingyl - flagYicdxh    - flagPandljsl - flagDaijf	
            //盘点差异 =（该零件消耗的）上次盈余 - (内部要货令的待交付-（JL毛需求的待消耗-sppv消耗）+（线边盘点）零件数量) 
     		rowElement.find('td:eq(' + chayIndex + ')').text(flagYingyl-(flagDaijf-(flagDaixh-flagXiaohl)+flagPandljsl));
     	}
     }
		...
		//调用交易
     	if (zhengclsh !=　""){
			...
			param.push('mos='+mos);
     		...
     		$.sdcui.ajaxUtil.ajax({
				url : "${ctx}/xinanxpd/checkLsh.ajax",
				data : param.join('&'),
				success : function(result) {
