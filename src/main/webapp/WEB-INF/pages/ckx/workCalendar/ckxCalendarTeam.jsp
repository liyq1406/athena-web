...
<t:html>
	...
	<t:page i18n="i18n.ckx.workCalendar.i18n_workCalendar">
		<t:grid id="grid_CkxCalendarTeam" 
			...
			src="queryCkxCalendarTeam.ajax"
			editAlias="editList"
			delAlias="delList"
			saveRowsSrcs="saveCkxCalendarTeam.ajax"
			load="false"
scrollHeight="335" pageSize="15"
			editable="true" showNum="true">
			<t:fieldLayout prefix="record0" columns="4">
			<t:fieldSelect src="getSelectTeamCode.ajax" show="bianzh" code="bianzh"  property="bianzh"  caption="${i18n.bianzh}" width="110"/>
				<t:fieldSelect property="xingqxh" caption="${i18n.xingqxh}" convert="queryXQ" width="110"/>
				<t:fieldText property="xuh" caption="${i18n.xuhao}" maxLength="2" width="110"/>
				<t:fieldText property="ban" dataType="text" maxLength="1"  minLength="1" caption="${i18n.ban}" width="110"/>
				<t:fieldText property="kaissj" expression="^(0[0-9]{1}|1[0-9]{1}|2[0-3]):[0-5][0-9]{1}:([0-5][0-9]{1})$" maxLength="8" minLength="8" expressionMessage="${i18n.qingtx8wsjgs}" caption="${i18n.kaissj}" width="110"/>
				<t:fieldText property="jiezsj" expression="^(0[0-9]{1}|1[0-9]{1}|2[0-3]):[0-5][0-9]{1}:([0-5][0-9]{1})$" maxLength="8" minLength="8" expressionMessage="${i18n.qingtx8wsjgs}" caption="${i18n.jiezsj}" width="110"/>
					<t:fieldSelect property="biaos" caption="${i18n.biaos}"
				convert="biaos" defaultValue="1" width="110"></t:fieldSelect>
		...
		<!-- 添加form -->
		<t:form dialog="true" id="form_addCkxCalendarTeam" reset="NOT" caption="${i18n.gongzsj}" action="addTeam.ajax">
		...
		<!-- 复制form -->
		<t:form dialog="true" id="form_copyCkxCalendarTeam" reset="NOT" caption="${i18n.gongzsj}" action="copyTeam.ajax">
		...
		<!-- 删除form -->
		<t:form dialog="true"  id="form_delCkxCalendarTeam" reset="NOT" caption="${i18n.gongzsj}" action="delTeam.ajax">
	...
	<script type="text/javascript">
			...
			function func_grid_physicalCalendarTeam(){
				...
				if(confirm("${i18n.niqdysc}")){					
					$.sdcui.ajaxUtil.ajax({
						url : "removeCkxCalendarTeam.ajax?bianzh="+record.bianzh+"&xingqxh="+record.xingqxh+"&xuh="+record.xuh,
						success : function() {
			...
			/* function func_button_submit(){
				alert();
				$('#form_addCkxCalendarTeam').form('submit');
			...
			function form_addCkxCalendarTeam_afterSubmit(result){
				if(result.result.message!="${i18n.yonghzwq}"){
				...
				if("checked" == $("#checkbox").attr("checked")){
					...
					$('#field-pop-record1_bianzh').removeClass("loaded");//刷新表格数据
