...
<t:page i18n="i18n.ckx.xuqjs.i18n_lingjxhd" >
	<t:grid id="grid_jipxx" caption="集配信息" 
		src="/xuqjs/queryJipxx.ajax"
		idKeys="usercenter,lingjbh,xiaohdbh"  
		load="false" 
		dataFormId="form_jipxx"
		back="NOT"
		editable="NOT" 
		showNum="true"
		add="NOT"
		...
		pageSize="15"
		>
		<t:fieldLayout id="jipxx" prefix="jipxx" columns="3">
   ...
   <t:form dialog="true" id="form_jipxx" caption="集配信息" action="/xuqjs/saveJipxx.ajax" width="660" 
	...
	<c:if test="${ 'ZBC' == role }">
	<t:file action="/xuqjs/importAth_uploadJipxx.do" size="50">
		<t:fileparam sheet="jipxx" keys="USERCENTER,LINGJBH,XIAOHDBH" table="{dbSchemal3}CKX_LINGJXHD" datasourceId="1"/>
	</t:file>
	</c:if>
</t:page>
...
<script type="text/javascript">
		...
		//空格查询
		function grid_jipxx_beforeSubmit() {
			$("#jipxx_lingjbh").fieldValue(trim($("#jipxx_lingjbh").fieldValue()));
			$("#jipxx_xiaohdbh").fieldValue(trim($("#jipxx_xiaohdbh").fieldValue()));
			$("#jipxx_peislxbh").fieldValue(trim($("#jipxx_peislxbh").fieldValue()));
		}
		...
		function func_button_edit() {
			...
			$('#form_jipxx').form("clear");//清除弹出窗口的数据
			
			$('#form_jipxx').removeClass('operant-add');//增加的operant为1，修改的operant为2
			
			var record = $("#grid_jipxx").grid("getSelectedRecords")[0];//选中的行的数据
...
// 					$('#form_jipxx').form('fillRecord', $.extend({}, record));
// 					var r = result.result ;
// 					$('#form_jipxx').form('fillRecord', $.extend({}, r));
// 					$("#form_jipxx").form("open","修改");
// 				}
// 			});
 					$('#form_jipxx').form('fillRecord', $.extend({}, record));
 					$("#form_jipxx").form("open","修改");
		...
		function func_button_save() {
			if($("#grid_jipxx").grid("isEditedCell")){
				$('#grid_jipxx').grid('saveRows');
			}
		}
