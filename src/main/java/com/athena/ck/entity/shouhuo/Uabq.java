package com.athena.ck.entity.shouhuo;

import java.util.Date;

/**
 * <p>
 * Title:UA标签
 * </p>
 * <p>
 * Description:定义UA标签实体类
 * </p>
 * <p>
 * Copyright: Copyright (c) 2011
 * </p>
 * <p>
 * Company: 软通动力
 * </p>
 * 
 * @author 林笛
 * @version v1.0
 * @date 2011-12-26上午10:56:58
 */
public class Uabq {
	private String usercenter;
	private String uth;
	private String uah;
	private String ulh;
	private String blh;
	private String elh;
	private String gongysdm;
	private String gongysmc;
	private char gongyslx;
	private String pich;
	private String uaxh;
	private long uarl;
	private String ucxh;
	private long ucgs;
	private String uchl;
	private String lingjbh;
	private String lingjmc;
	private String cangkbh;
	private String zickbh;
	private String danw;
	private Double lingjsl;
	private String baozzt;
	private String zhuangtsx;
	private Date chuangjrq;
	private Date shixrq;
	private String chanx;
	private long jingz;
	private long maoz;
	private Date daysj;
	private String beiz;
	private String yicm;
	private long yanssl;
	private String yansts;
	private String yaohlh;
	private String dingdh;
	private String caozy;
	private String yansy;
	private Date ruksj;
	private String jihydm;
	
	//xss-20160817
	private long iBaozgs;
	private long ljsj;
	private String shifdyus;
	private long uasl;
	private long ucsl;
	private String tishi;
	private String xuh;
	
	
	
	
	
	public String getXuh() {
		return xuh;
	}

	public void setXuh(String xuh) {
		this.xuh = xuh;
	}

	public long getUasl() {
		return uasl;
	}

	public void setUasl(long uasl) {
		this.uasl = uasl;
	}

	public long getUcsl() {
		return ucsl;
	}

	public void setUcsl(long ucsl) {
		this.ucsl = ucsl;
	}

	public String getTishi() {
		return tishi;
	}

	public void setTishi(String tishi) {
		this.tishi = tishi;
	}

	public long getiBaozgs() {
		return iBaozgs;
	}

	public void setiBaozgs(long iBaozgs) {
		this.iBaozgs = iBaozgs;
	}

	public long getLjsj() {
		return ljsj;
	}

	public void setLjsj(long ljsj) {
		this.ljsj = ljsj;
	}

	public String getShifdyus() {
		return shifdyus;
	}

	public void setShifdyus(String shifdyus) {
		this.shifdyus = shifdyus;
	}

	public long getUss() {
		return uss;
	}

	public void setUss(long uss) {
		this.uss = uss;
	}

	public String getUtscsj() {
		return utscsj;
	}

	public void setUtscsj(String utscsj) {
		this.utscsj = utscsj;
	}

	private char kajthbs;
	private char kanthbs;
    private String blh1;
    
    //huxy_11459
    private long uss;
    private String utscsj;
    
	public String getBlh1() {
		return blh1;
	}

	public void setBlh1(String blh1) {
		this.blh1 = blh1;
	}

	/**
	 * 返回 用户中心
	 * 
	 * @return the usercenter
	 */
	public String getUsercenter() {
		return usercenter;
	}

	/**
	 * 设置 用户中心
	 * 
	 * @param usercenter
	 *            the usercenter to set
	 */
	public void setUsercenter(String usercenter) {
		this.usercenter = usercenter;
	}

	/**
	 * 返回 UT号
	 * 
	 * @return the uth
	 */
	public String getUth() {
		return uth;
	}

	/**
	 * 设置 UT号
	 * 
	 * @param uth
	 *            the uth to set
	 */
	public void setUth(String uth) {
		this.uth = uth;
	}

	/**
	 * 返回 UA号
	 * 
	 * @return the uah
	 */
	public String getUah() {
		return uah;
	}

	/**
	 * 设置 UA号
	 * 
	 * @param uah
	 *            the uah to set
	 */
	public void setUah(String uah) {
		this.uah = uah;
	}

	/**
	 * 返回 UL号
	 * 
	 * @return the ulh
	 */
	public String getUlh() {
		return ulh;
	}

	/**
	 * 设置 UL号
	 * 
	 * @param ulh
	 *            the ulh to set
	 */
	public void setUlh(String ulh) {
		this.ulh = ulh;
	}

	/**
	 * 返回 BL号
	 * 
	 * @return the blh
	 */
	public String getBlh() {
		return blh;
	}

	/**
	 * 设置 BL号
	 * 
	 * @param blh
	 *            the blh to set
	 */
	public void setBlh(String blh) {
		this.blh = blh;
	}

	/**
	 * 返回 EL号
	 * 
	 * @return the elh
	 */
	public String getElh() {
		return elh;
	}

	/**
	 * 设置 EL号
	 * 
	 * @param elh
	 *            the elh to set
	 */
	public void setElh(String elh) {
		this.elh = elh;
	}

	/**
	 * 返回 供应商编号
	 * 
	 * @return the gongysdm
	 */
	public String getGongysdm() {
		return gongysdm;
	}

	/**
	 * 设置 供应商编号
	 * 
	 * @param gongysdm
	 *            the gongysdm to set
	 */
	public void setGongysdm(String gongysdm) {
		this.gongysdm = gongysdm;
	}

	/**
	 * 返回 供应商名称
	 * 
	 * @return the gongysmc
	 */
	public String getGongysmc() {
		return gongysmc;
	}

	/**
	 * 设置 供应商名称
	 * 
	 * @param gongysmc
	 *            the gongysmc to set
	 */
	public void setGongysmc(String gongysmc) {
		this.gongysmc = gongysmc;
	}

	/**
	 * 返回 供应商类型
	 * 
	 * @return the gongyslx
	 */
	public char getGongyslx() {
		return gongyslx;
	}

	/**
	 * 设置 供应商类型
	 * 
	 * @param gongyslx
	 *            the gongyslx to set
	 */
	public void setGongyslx(char gongyslx) {
		this.gongyslx = gongyslx;
	}

	/**
	 * 返回 批次号
	 * 
	 * @return the pich
	 */
	public String getPich() {
		return pich;
	}

	/**
	 * 设置 批次号
	 * 
	 * @param pich
	 *            the pich to set
	 */
	public void setPich(String pich) {
		this.pich = pich;
	}

	/**
	 * 返回 UA型号
	 * 
	 * @return the uaxh
	 */
	public String getUaxh() {
		return uaxh;
	}

	/**
	 * 设置 UA型号
	 * 
	 * @param uaxh
	 *            the uaxh to set
	 */
	public void setUaxh(String uaxh) {
		this.uaxh = uaxh;
	}

	/**
	 * 返回 UA容量
	 * 
	 * @return the uarl
	 */
	public long getUarl() {
		return uarl;
	}

	/**
	 * 设置 UA容量
	 * 
	 * @param uarl
	 *            the uarl to set
	 */
	public void setUarl(long uarl) {
		this.uarl = uarl;
	}

	/**
	 * 返回 UC型号
	 * 
	 * @return the ucxh
	 */
	public String getUcxh() {
		return ucxh;
	}

	/**
	 * 设置 UC型号
	 * 
	 * @param ucxh
	 *            the ucxh to set
	 */
	public void setUcxh(String ucxh) {
		this.ucxh = ucxh;
	}

	/**
	 * 返回 UC个数
	 * 
	 * @return the ucgs
	 */
	public long getUcgs() {
		return ucgs;
	}

	/**
	 * 设置 UC个数
	 * 
	 * @param ucgs
	 *            the ucgs to set
	 */
	public void setUcgs(long ucgs) {
		this.ucgs = ucgs;
	}

	/**
	 * 返回 UC含量
	 * 
	 * @return the uchl
	 */
	public String getUchl() {
		return uchl;
	}

	/**
	 * 设置 UC含量
	 * 
	 * @param uchl
	 *            the uchl to set
	 */
	public void setUchl(String uchl) {
		this.uchl = uchl;
	}

	/**
	 * 返回 零件编号
	 * 
	 * @return the lingjbh
	 */
	public String getLingjbh() {
		return lingjbh;
	}

	/**
	 * 设置 零件编号
	 * 
	 * @param lingjbh
	 *            the lingjbh to set
	 */
	public void setLingjbh(String lingjbh) {
		this.lingjbh = lingjbh;
	}

	/**
	 * 返回 零件名称
	 * 
	 * @return the lingjmc
	 */
	public String getLingjmc() {
		return lingjmc;
	}

	/**
	 * 设置 零件名称
	 * 
	 * @param lingjmc
	 *            the lingjmc to set
	 */
	public void setLingjmc(String lingjmc) {
		this.lingjmc = lingjmc;
	}

	/**
	 * 返回 仓库编号
	 * 
	 * @return the cangkbh
	 */
	public String getCangkbh() {
		return cangkbh;
	}

	/**
	 * 设置 仓库编号
	 * 
	 * @param cangkbh
	 *            the cangkbh to set
	 */
	public void setCangkbh(String cangkbh) {
		this.cangkbh = cangkbh;
	}

	/**
	 * 返回 子仓库编号
	 * 
	 * @return the zickbh
	 */
	public String getZickbh() {
		return zickbh;
	}

	/**
	 * 设置 子仓库编号
	 * 
	 * @param zickbh
	 *            the zickbh to set
	 */
	public void setZickbh(String zickbh) {
		this.zickbh = zickbh;
	}

	/**
	 * 返回 单位
	 * 
	 * @return the danw
	 */
	public String getDanw() {
		return danw;
	}

	/**
	 * 设置 单位
	 * 
	 * @param danw
	 *            the danw to set
	 */
	public void setDanw(String danw) {
		this.danw = danw;
	}

	/**
	 * 返回 零件数量
	 * 
	 * @return the lingjsl
	 */
	public Double getLingjsl() {
		return lingjsl;
	}

	/**
	 * 设置 零件数量
	 * 
	 * @param lingjsl
	 *            the lingjsl to set
	 */
	public void setLingjsl(Double lingjsl) {
		this.lingjsl = lingjsl;
	}

	/**
	 * 返回 包装状态
	 * 
	 * @return the baozzt
	 */
	public String getBaozzt() {
		return baozzt;
	}

	/**
	 * 设置 包装状态
	 * 
	 * @param baozzt
	 *            the baozzt to set
	 */
	public void setBaozzt(String baozzt) {
		this.baozzt = baozzt;
	}

	/**
	 * 返回 状态属性
	 * 
	 * @return the zhuangtsx
	 */
	public String getZhuangtsx() {
		return zhuangtsx;
	}

	/**
	 * 设置 状态属性
	 * 
	 * @param zhuangtsx
	 *            the zhuangtsx to set
	 */
	public void setZhuangtsx(String zhuangtsx) {
		this.zhuangtsx = zhuangtsx;
	}

	/**
	 * 返回 创建日期
	 * 
	 * @return the chuangjrq
	 */
	public Date getChuangjrq() {
		return chuangjrq;
	}

	/**
	 * 设置 创建日期
	 * 
	 * @param chuangjrq
	 *            the chuangjrq to set
	 */
	public void setChuangjrq(Date chuangjrq) {
		this.chuangjrq = chuangjrq;
	}

	/**
	 * 返回 失效日期
	 * 
	 * @return the shixrq
	 */
	public Date getShixrq() {
		return shixrq;
	}

	/**
	 * 设置 失效日期
	 * 
	 * @param shixrq
	 *            the shixrq to set
	 */
	public void setShixrq(Date shixrq) {
		this.shixrq = shixrq;
	}

	/**
	 * 返回 产线
	 * 
	 * @return the chanx
	 */
	public String getChanx() {
		return chanx;
	}

	/**
	 * 设置 产线
	 * 
	 * @param chanx
	 *            the chanx to set
	 */
	public void setChanx(String chanx) {
		this.chanx = chanx;
	}

	/**
	 * 返回 净重
	 * 
	 * @return the jingz
	 */
	public long getJingz() {
		return jingz;
	}

	/**
	 * 设置 净重
	 * 
	 * @param jingz
	 *            the jingz to set
	 */
	public void setJingz(long jingz) {
		this.jingz = jingz;
	}

	/**
	 * 返回 毛重
	 * 
	 * @return the maoz
	 */
	public long getMaoz() {
		return maoz;
	}

	/**
	 * 设置 毛重
	 * 
	 * @param maoz
	 *            the maoz to set
	 */
	public void setMaoz(long maoz) {
		this.maoz = maoz;
	}

	/**
	 * 返回 打印时间
	 * 
	 * @return the daysj
	 */
	public Date getDaysj() {
		return daysj;
	}

	/**
	 * 设置 打印时间
	 * 
	 * @param daysj
	 *            the daysj to set
	 */
	public void setDaysj(Date daysj) {
		this.daysj = daysj;
	}

	/**
	 * 返回 备注
	 * 
	 * @return the beiz
	 */
	public String getBeiz() {
		return beiz;
	}

	/**
	 * 设置 备注
	 * 
	 * @param beiz
	 *            the beiz to set
	 */
	public void setBeiz(String beiz) {
		this.beiz = beiz;
	}

	/**
	 * 返回 异常码
	 * 
	 * @return the yicm
	 */
	public String getYicm() {
		return yicm;
	}

	/**
	 * 设置 异常码
	 * 
	 * @param yicm
	 *            the yicm to set
	 */
	public void setYicm(String yicm) {
		this.yicm = yicm;
	}

	/**
	 * 返回 验收数量
	 * 
	 * @return the yanssl
	 */
	public long getYanssl() {
		return yanssl;
	}

	/**
	 * 设置 验收数量
	 * 
	 * @param yanssl
	 *            the yanssl to set
	 */
	public void setYanssl(long yanssl) {
		this.yanssl = yanssl;
	}

	/**
	 * 返回 验收提示
	 * 
	 * @return the yansts
	 */
	public String getYansts() {
		return yansts;
	}

	/**
	 * 设置 验收提示
	 * 
	 * @param yansts
	 *            the yansts to set
	 */
	public void setYansts(String yansts) {
		this.yansts = yansts;
	}

	/**
	 * 返回 要货令号
	 * 
	 * @return the yaohlh
	 */
	public String getYaohlh() {
		return yaohlh;
	}

	/**
	 * 设置 要货令号
	 * 
	 * @param yaohlh
	 *            the yaohlh to set
	 */
	public void setYaohlh(String yaohlh) {
		this.yaohlh = yaohlh;
	}

	/**
	 * 返回 订单号
	 * 
	 * @return the dingdh
	 */
	public String getDingdh() {
		return dingdh;
	}

	/**
	 * 设置 订单号
	 * 
	 * @param dingdh
	 *            the dingdh to set
	 */
	public void setDingdh(String dingdh) {
		this.dingdh = dingdh;
	}

	/**
	 * 返回 操作员
	 * 
	 * @return the caozy
	 */
	public String getCaozy() {
		return caozy;
	}

	/**
	 * 设置 操作员
	 * 
	 * @param caozy
	 *            the caozy to set
	 */
	public void setCaozy(String caozy) {
		this.caozy = caozy;
	}

	/**
	 * 返回 验收员
	 * 
	 * @return the yansy
	 */
	public String getYansy() {
		return yansy;
	}

	/**
	 * 设置 验收员
	 * 
	 * @param yansy
	 *            the yansy to set
	 */
	public void setYansy(String yansy) {
		this.yansy = yansy;
	}

	/**
	 * 返回 入库时间
	 * 
	 * @return the ruksj
	 */
	public Date getRuksj() {
		return ruksj;
	}

	/**
	 * 设置 入库时间
	 * 
	 * @param ruksj
	 *            the ruksj to set
	 */
	public void setRuksj(Date ruksj) {
		this.ruksj = ruksj;
	}

	/**
	 * 返回 计划员代码
	 * 
	 * @return the jihydm
	 */
	public String getJihydm() {
		return jihydm;
	}

	/**
	 * 设置 计划员代码
	 * 
	 * @param jihydm
	 *            the jihydm to set
	 */
	public void setJihydm(String jihydm) {
		this.jihydm = jihydm;
	}

	/**
	 * 返回 卡间跳号标识
	 * 
	 * @return the kajthbs
	 */
	public char getKajthbs() {
		return kajthbs;
	}

	/**
	 * 设置 卡间跳号标识
	 * 
	 * @param kajthbs
	 *            the kajthbs to set
	 */
	public void setKajthbs(char kajthbs) {
		this.kajthbs = kajthbs;
	}

	/**
	 * 返回 卡内跳号标识
	 * 
	 * @return the kanthbs
	 */
	public char getKanthbs() {
		return kanthbs;
	}

	/**
	 * 设置 卡内跳号标识
	 * 
	 * @param kanthbs
	 *            the kanthbs to set
	 */
	public void setKanthbs(char kanthbs) {
		this.kanthbs = kanthbs;
	}

}
